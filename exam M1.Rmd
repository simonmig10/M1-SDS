...
```
We can see the elbow shows the optimal dimensions are three dimensions with almost 65% explained variance. If we only look at the eigenvalues then our rule of thumb is to pick the dimensions with an eigenvalue >=1, which in this case is two dimensions. Those two dimensions only account for 52% of the total variance, which isn't partically high.
    Then we visualize our reduced data in two dimensions.
