...
Her indeholder "hc_func" et "stand = TRUE"-argument, hvorfor data ikke skal skaleres f?rst. 
```{r}
res_hc <- data %>%
...
*In the dendrogram every data point finally merges into a single cluster with the height(distance) shown on the y-axis.* 
```{r}
res_hc %>%
...
Vi kan igen lave en krydstabel mellem clusters og penguins, hvor vi kan observerer at Adelie udelukkende optr?der i cluster 1. Dette stemmer samtidig overens med 2d-plottet. Det observeres dermed at "hierarchical clustering" er en smule bedre end "K-means clustering". 
```{r}
...
*Combine a PCA with clustering techniques.* 
```{r}
res_hcpc <- res_pca %>% 
...
Vi kan her lave et 3d-plot for clusters. 
```{r}
res_hcpc %>%
  plot(choice = "3D.map")
