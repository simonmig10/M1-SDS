...
A **training dataset** is a dataset of examples used during the learning process and is used to fit the models. 
...
Laver lige status til numerisk. Dette gør vi fordi der kom fejl længere nede.
...
data_y_num %>% 
  mutate(y = ifelse(y == "closed", 0, 1))
```

```{r}
data_split_stat_num <- initial_split(data_y_num, prop = 0.75, strata = y)

data_train_num <- data_split_stat_num  %>%  training()
data_test_num <- data_split_stat_num %>% testing()
```



Laver lige recipe igen med dummy i stedet

```{r}

data_recipe <- data_train_num %>%
  recipe(y ~.) %>%
  step_log(funding_total_usd) %>% #Myb just delete this one
  step_center(all_numeric(), -all_outcomes()) %>%
  step_scale(all_numeric(), -all_outcomes()) %>%
...
```{r}
data_train_num #75% af observationer
data_test_num #25% af observationer
...
Først tilføjes latitude og longitude.

```{r}
data2 <- read_csv("startup data.csv", 
         col_types = cols(founded_at = col_date(format = "%m/%d/%Y"), 
                          first_funding_at = col_date(format = "%m/%d/%Y"), 
                          last_funding_at = col_date(format = "%m/%d/%Y")))

data2 %<>%
  select(!c(`Unnamed: 0`, `Unnamed: 6`, state_code.1, object_id, avg_participants, has_roundA, has_roundB, has_roundC, has_roundD, zip_code, id, city, name, labels))

```

Fordi vi ikke vil ændre på dataet, vi vil bare lege lidt med det.
```{r}
data_explore <- data2
```

Nu laver vi et kort over observationerne adskilt med farve ift. status
```{r}
data_explore %>% 
  ggplot(aes(x = longitude, y = latitude)) +
  geom_point(aes(size = 1, color = status), 
             alpha = 0.4)
```
...
qmplot(x = longitude, 
       ...
       geom = "point", 
       color = status, 
       size = 0.1, #kan vi godt opdele efter en kategori også
       alpha = 0.4) +
  scale_alpha(guide = 'none') # don't show legend for alpha
```
Som om vi er blevet vildledt.


## Validation set

Using only two partitions may be insufficient when doing many rounds of hyperparameter tuning (which we don’t perform in this tutorial but it is always recommended to use a validation set).

Derfor laver vi et validation set
...
## Create workflows
A workflow is an object that can bundle together your pre-processing recipe, modeling, and even post-processing requests (like calculating the RMSE).

### Decision tree

```{r}
d_tree_wflow <- # new workflow object
 workflow() %>% # use workflow function
 add_recipe(data_recipe) %>%   # use the new recipe
 add_model(dt_spec)   # add your model spec
...
Vi har 3 recipe steps mindre end de har
...
Den der over her var den der ikke fungerede med string
...
```
It looks like the Random Forest is the best! 
...
```
## We now use the best model on test data
...
last_fit_rf %>% 
  ...
  autoplot()
```
Det er nok ik verdens bedste model, men heller ik verdens væreste! 
...
pred_dt %>%
  mutate(pred_dt = filter(pred_dt == ))
#data(ROCR.simple)
#df <- data.frame(ROCR.simple)
pred_dt_2 <- prediction(pred_dt, truth_dt)
