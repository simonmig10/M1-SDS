...
```{r}
plot_trips = trips %>%
  drop_na(sub_region, trip_durration_days) %>%
  filter(sub_region == "Western Europe") %>% 
  group_by(country) %>% 
  summarise(total_time = sum(trip_durration_days))
...
trips %>% 
 ...
 filter(grepl("Startup Founder", work_raw)) %>% 
  summarise(avg_time = mean(trip_durration_days))
...
trips %>%
  drop_na(region) %>% 
  mutate(trip_durration_week = trip_durration_days/7) %>%
  group_by(region) %>% 
  summarise(median = median(trip_durration_week)) %>% 
