...
trips_98_days=trips %>%
  mutate(trip_duration_days_rank= percent_rank(trip_duration_days))%>%
  arrange(desc(trip_duration_days_rank))%>%
  filter(trip_duration_days_rank < 0.98)%>%
  select(trip_duration_days_rank, trip_duration_days, everything())


trips_98_end %>%
  mutate(date_end_time_rank= percent_rank(date_end_time))%>%
  arrange(desc(trip_duration_days_rank))%>%
  filter(trip_duration_days_rank < 0.98)%>%
  select(trip_duration_days_rank, trip_duration_days, everything())
...
trimed_trips=trips %>%
  filter(trip_duration_days >= 1 & trip_duration_days <= 97, date_end_time >= quantile(date_end_time, 0.01) & date_end_time <= quantile(date_end_time, 0.97))
