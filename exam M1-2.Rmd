...
```
Lav de plots her efter vi har fjernet NA's
...
We start by removing columns there are undefined when loading the dataset and columns which show the same things. We also remove variables like longtitude and latitude, which doesnt seem to bring much insight to the ongoing analysis. 
...
**If we want to remove outliers do it here**

Maybe do a boxplot of the numerical variables we use
...
data %>% 
  ggplot(aes(funding_total_usd))+
  geom_histogram()

data%>% arrange(desc(funding_total_usd))

data %<>%
  filter(funding_total_usd < 5700000000)
  #Changes conclusion and makes the supervised model better. 
...
We start by performing dimensionalty reduction on our data, but before we can do that, we need to examine our data to figure out if we need to scale it?
...
**Myb do one more plot with different elipses could make a categorical variable from the starting data**

```{r}

uml_elipse_data=data_start%>%
  filter(funding_total_usd < 5700000000 )%>%
  mutate(state_code= ifelse(state_code %in% c("CA", "MA", "NY", "TX"), state_code, "other"))
...
res_pca %>%
fviz_pca_biplot(alpha.ind = "cos2",
habillage = uml_elipse_data %>% pull(state_code) %>% factor(),
addEllipses = TRUE,
geom = "point",
ggtheme = theme_gray())
...
UMAP seem to separate the observations better as we kinda have 4 clusters here. But they are not separated that well between closed and acquired. It does however seem like most of the firms which has been acquired **(closed right?)** is to the left and the top left of the plot.
...
**Kan ik huske men er der en måde at finde optimale mængde cluster her?**
...
Just to get an idea how the y variable is split between aquaried and closed we can do a histogram
...
step_log bruges hvis variablen er lidt scewed!
...
#### We use Random Forrest on the test data
