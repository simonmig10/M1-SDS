...
We start by looking at the data with the head function

```{r}
pokemon %>% head()
```

We can also skim the data and see we got 3 categrical, 1 logical and 8 numeric variables. 
...
```
When we talk about scaling variables we need to only focus on the variables we are able to scale. 

*Character strings:* We can see the data has 3 character strings "names" which are the names of the pokemons, "type1" which is the main type of the pokemon, and last "type2" which shows some pokemons has a second type, where NA's means they only have one type. These columns wont need scaling. 
...
We can see Generation is numeric and show which generation of pokemon game each pokemon is from. So we will also exclude this one as it looks more like an categrical variable. 
...
We now have an overwiev over the data, and are able to see which variables it would make since to include in a dimensionality reduction and if it would be a good idea to scale them. 
...
We run a PCA on the pokemon dataset with scale equal to TRUE because of the "total" column. Then we make a scree plot to pick number of dimensions to use. We remove Generation and the non numerical variables. 
...
To answer the latter of the question regarding the cumulative variance using 4 components we simply extract the cumulative variance calculated in the pca analysis.

```{r}
res_pca$eig[,3][1:4]
```

We can see that the cumulative variance at component 4 is 90.06% This means the 4 dimensions explain 90.06% of the variance in the data.
...
It looks like UMAP are separating the legendary pokemon from the normal pokemon a bit better then the PCA analysis by clustering them closer together (the blue observations).
...
We start by selecting all our numerical variables except Generations (as we clasified this as categorical) and run a Kmean cluster analysis.
...
We then use the kmeans function to seperate the data into two clusters. 
...
res_km 
```
As we are gonna use some of the categorical variables later we will now create a new variable called pokemon_nr only including numeric variables (exc. Generation)
```{r}
pokemon_nr = pokemon %>% 
  select(!Generation) %>% 
  select(is.numeric) 
...
```{r,,fig.width=15,fig.height=10,fig.align='center', warning=FALSE}
res_km %>% 
  fviz_cluster(data = pokemon_nr %>% select_if(is_numeric) ,
               ggtheme = theme_gray())  
```

After using KMEANS on the scaled and not dimentionaly reduced data, we get two clusters which are not overlapping and looks nice.

**Visualize the first 2 principal components and color the datapoints by cluster.**

We add the PCA columns to the dataset
...
To still keep the normal datasets we create two new dataset called clusters and add the cluster notation (1 or 2) to the observations. 
...
```
This looks very much like the same plot from before. 
...
We can see the two clusters are mainly seperatet by overall attribute and not the type of the pokemon. (we can note this by looking at the mean of each attribute in the 2 clusters)
...
We have visualized the PCA analysis in the question above using two dimensions. And will now use the same steps in the above question uising K-means now only using the two columns showing the 2 dimensions from the PCA analysis.
...
We can see it has split the observations into 3 clusters very nicely. We assign the cluster values to the observations
...
```
Again the plot looks exactly the same. 
